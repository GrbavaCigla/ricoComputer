...
use rc_common::{SyntaxTree, Error};
...
pub fn parse(input: &str) -> Result<SyntaxTree, Error> {
    let label_prefix = cmws(
        terminated(ws(label), opt(comment(line_ending))),
        line_ending,
    );
    ...
    let decl_line = terminated(
        ws(decl),
        empty_line(opt_comment_end(line_ending), line_ending),
    );
